...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def on_pushButton_3_clicked(self):
        ...
        file_name = QtGui.QFileDialog.getOpenFileName(self, u'选择图像', r"/home/nikoong/Algorithm_test/PyQt/gui/same_person_result/query")
        print "1"
        """
        try:
            file = open(file_name)
    ...
    def on_pushButton_2_clicked(self):
        ...
        if self.comboBox.currentText() == "1" and self.comboBox_2.currentText() == "1":
            ...
            #
            #这里操作根据实际需完善：                                
            #1、需要完善的内容：按下搜索操作后的逻辑，包括进行特征生成和比对操作 
            #2、输入输出：输入是已经得到的query_pathfile_name、输出是camb_imgs_list
            #3、目前函数模型和参数是逻辑内置固定的。UI不提供选择操作。
            #
            ########################################

            #上面的操作得到的最后的结果应该是一个camb_imgs_list
            #最后得到camb_imgs_list。list的前50个元素是最相似度的50位候选者的图片。
            camb_imgs_list = []
